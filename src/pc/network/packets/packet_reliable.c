...
void network_forget_all_reliable(void) {
    ...
    while (node != NULL) {
        ...
        if (!node->p.keepSendingAfterDisconnect) { remove_node_from_list(head); }
        node = next;
    }
}
...
void network_forget_all_reliable_from(u8 localIndex) {
    ...
    while (node != NULL) {
        ...
        if (node->p.localIndex == localIndex) {
            if (!node->p.keepSendingAfterDisconnect) { remove_node_from_list(node); }
        }
        node = next;
    }
}
    ...
    while (node != NULL) {
        ...
        if (elapsed > maxElapsed) {
            ...
            int maxResendAttempts =
                node->p.packetType == PACKET_MOD_LIST_REQUEST ? 60 : MAX_RESEND_ATTEMPTS;
