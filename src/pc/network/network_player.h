...
/* |description|Gets the first network player whose information matches `courseNum`, `actNum`, and
 * `levelNum`|descriptionEnd| */
struct NetworkPlayer *get_network_player_from_level(s16 courseNum, s16 actNum, s16 levelNum);
/* |description|Gets the first network player whose information matches `courseNum`, `actNum`,
 * `levelNum`, and `areaIndex`|descriptionEnd| */
struct NetworkPlayer *get_network_player_from_area(s16 courseNum, s16 actNum, s16 levelNum,
                                                   s16 areaIndex);
/* |description|Gets the active network player with the smallest global index. Useful for assigning one
 * player to "own" some kind of functionality or object|descriptionEnd| */
...
/* |description|Gets a red, green, or blue value from a part in `np`'s override color
 * palette|descriptionEnd| */
...
/* |description|Checks if `np`'s override color palette is identical to the regular color
 * palette|descriptionEnd| */
...
u8 network_player_connected(enum NetworkPlayerType type, u8 globalIndex, u8 modelIndex,
                            const struct PlayerPalette *playerPalette, const char *name,
