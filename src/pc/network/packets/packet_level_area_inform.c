...
void network_send_level_area_inform(void) {
    ...
    LOG_INFO("tx level area inform for global %d: (%d, %d, %d, %d)", np->globalIndex, np->currCourseNum,
             np->currActNum, np->currLevelNum, np->currAreaIndex);
}
...
void network_receive_level_area_inform(struct Packet *p) {
    ...
    LOG_INFO("rx level area inform for global %d: (%d, %d, %d, %d)", globalIndex, courseNum, actNum,
             levelNum, areaIndex);
