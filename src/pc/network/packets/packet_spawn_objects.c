...
static u32 generate_parent_id(struct Object *objects[], u8 onIndex, bool sanitize) {
    ...
    if (onIndex == 0) {
        if (!o->parentObj) { return (u32)-1; }
        if (sanitize && o->parentObj->oSyncID == 0) { return (u32)-1; }
...
void network_send_spawn_objects_to(u8 sendToLocalIndex, struct Object *objects[], u32 models[],
                                   u8 objectCount) {
    ...
    if (sendToLocalIndex == PACKET_DESTINATION_BROADCAST) {
        ...
        if (objects[0] && objects[0]->behavior) {
            LOG_INFO("tx spawn objects to %d | %u", gNetworkPlayers[sendToLocalIndex].globalIndex,
                     get_id_from_behavior(objects[0]->behavior));
        }
    }
}
        ...
        // Don't overwrite existing sync objects
        {
            ...
            if (so && so->o) {
                if (so->o->behavior == get_behavior_from_id(data.behaviorId)) {
                    LOG_ERROR("recieved duplicate sync object with id %d from %s (%s)", syncID, name,
                              id);
                } else {
                    LOG_ERROR("recieved duplicate sync object with id %d with different behavior %s "
                              "from %s (%s)",
                              syncID, bhvName, name, id);
                }
                continue;
            }
        }
