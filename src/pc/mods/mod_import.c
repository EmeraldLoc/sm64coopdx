...
static bool mod_import_zip(char* path, bool* isLua, bool* isDynos) {
    ...
    *isDynos = false;
    for (int i = 0; i < (int)mz_zip_reader_get_num_files(&zip_archive); i++) {
        ...
        if (path_ends_with(file_stat.m_filename, ".lua") || path_ends_with(file_stat.m_filename, ".luac") || path_ends_with(file_stat.m_filename, ".pluto") || path_ends_with(file_stat.m_filename, ".plutoc")) {
...
bool mod_import_file(char* path) {
    ...
    if (path_ends_with(path, ".lua") || path_ends_with(path, ".luac") || path_ends_with(path, ".pluto") || path_ends_with(path, ".plutoc")) {
        isLua = true;
