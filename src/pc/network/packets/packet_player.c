...
static void write_packet_data(struct PacketPlayerData *data, struct MarioState *m, u8 *customFlags,
                              u32 *heldSyncID, u32 *heldBySyncID, u32 *riddenSyncID,
                              u32 *interactSyncID, u32 *usedSyncID, u32 *platformSyncID) {
...
void network_receive_player(struct Packet *p) {
    ...
    if (gNetworkType == NT_SERVER
        && *((u32 *)(p->buffer + p->cursor + offsetof(struct PacketPlayerData, action)))
               == ACT_DEBUG_FREE_MOVE) {
#ifdef DEVELOPMENT
    ...
    bool levelAreaMismatch =
        ((gNetworkPlayerLocal == NULL) || np->currCourseNum != gNetworkPlayerLocal->currCourseNum
    ...
    if (oldData.action == ACT_JUMBO_STAR_CUTSCENE && data.action == ACT_JUMBO_STAR_CUTSCENE) { return; }
    ...
    u8 customFlags = 0;
    write_packet_data(&data, m, &customFlags, &heldSyncID, &heldBySyncID, &riddenSyncID,
                      &interactSyncID, &usedSyncID, &platformSyncID);
    ...
    if (m->heldObj != NULL) {
        if (gMarioStates[0].heldObj == m->heldObj
            && np->globalIndex < gNetworkPlayerLocal->globalIndex) {
    ...
    if (interactSo && interactSo->o) { m->interactObj = interactSo->o; }

    // find and set their used object
    struct SyncObject *usedSo = sync_object_get(usedSyncID);
    if (usedSo && usedSo->o != NULL) { m->usedObj = usedSo->o; }
    ...
    if (m->action == ACT_JUMP_KICK) { m->actionState = oldData.actionState; }

    // punching:
    if ((m->action == ACT_PUNCHING || m->action == ACT_MOVE_PUNCHING)) {
        // play first punching sound, otherwise it will be missed
        if (m->action != oldData.action) { play_character_sound(m, CHAR_SOUND_PUNCH_YAH); }
    ...
    if (m->action != oldData.action) { m->actionTimer = 0; }
    ...
    if (np->currLevelNum == LEVEL_BOWSER_3 && m->action == ACT_JUMBO_STAR_CUTSCENE
        && gMarioStates[0].action != ACT_JUMBO_STAR_CUTSCENE) {
...
void network_update_player(void) {
    ...
    u8 localIsHeadless =
        (&gNetworkPlayers[0] == gNetworkPlayerServer && gServerSettings.headlessServer);
    ...
    f32 stickDist = sqrtf(powf(sLastStickX - m->controller->stickX, 2)
                          + powf(sLastStickY - m->controller->stickY, 2));
    bool shouldSend = (sTicksSinceSend > 2) || (sLastPlayerAction != m->action)
                      ...
                      || (sLastPlayerParticles != m->particleFlags) || (stickDist > 5.0f);
