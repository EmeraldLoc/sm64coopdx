...
static int _debuglog_print_network_type(char *buffer, size_t bufferSize) {
    return snprintf(buffer, bufferSize, " [%02d] ",
                    (gNetworkPlayerLocal != NULL) ? gNetworkPlayerLocal->globalIndex : -1);
}
...
static inline void _debuglog_print_log(const char *color, const char *logType, const char *filename,
                                       const char *fmt, ...) {
...
#else
#define LOG_DEBUG(...)                                                                                 \
    (configDebugPrint ? (_debuglog_print_log("", "DEBUG", __FILE__, __VA_ARGS__)) : 0)
#define LOG_INFO(...)                                                                                  \
    ((configDebugInfo || gCLIOpts.headless) ? (_debuglog_print_log("", "INFO", __FILE__, __VA_ARGS__)) \
                                            : 0)
#define LOG_ERROR(...)                                                                                 \
    (configDebugError ? (_debuglog_print_log("\x1b[31m", "ERROR", __FILE__, __VA_ARGS__)) : 0)
#endif
#define LOG_CONSOLE(...)                                                                               \
    {                                                                                                  \
