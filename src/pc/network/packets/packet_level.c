...
void network_receive_level(struct Packet *p) {
    ...
    if (levelNum == LEVEL_TTC) {
        u32 sanityDepth = 0;
        for (struct Object *obj = obj_get_first(OBJ_LIST_SURFACE); obj != NULL;
             obj = obj_get_next(obj)) {
            if (++sanityDepth > 10000) { break; }
            if (obj_has_behavior_id(obj, id_bhvTTC2DRotator)) {
                if (obj->oBehParams2ndByte >= 0 && obj->oBehParams2ndByte < 2 && gTTCSpeedSetting >= 0
                    && gTTCSpeedSetting < 4) {
                    obj->oTTC2DRotatorMinTimeUntilNextTurn =
                        gTTC2DRotatorTimeBetweenTurns[obj->oBehParams2ndByte][gTTCSpeedSetting];
                ...
                if (gTTCSpeedSetting != TTC_SPEED_STOPPED && gTTCSpeedSetting > 0
                    && gTTCSpeedSetting < 4) {
            ...
            } else if (obj_has_behavior_id(obj, id_bhvTTCPitBlock)
                       && oldTTCSpeedSetting == TTC_SPEED_STOPPED
                ...
                *obj->oTTCTreadmillBigSurface = *obj->oTTCTreadmillSmallSurface =
                    gTTCTreadmillSpeeds[gTTCSpeedSetting];
            }
        }
    }
}
