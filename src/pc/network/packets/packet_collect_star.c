...
void network_receive_collect_star(struct Packet *p) {
    ...
    bool levelAreaMismatch =
        ((np == NULL) || np->currCourseNum != gCurrCourseNum || np->currActNum != gCurrActStarNum
         || np->currLevelNum != gCurrLevelNum || np->currAreaIndex != gCurrAreaIndex);
    ...
    if (!levelAreaMismatch) {
        ...
        if (star != NULL) { star->oInteractStatus = INT_STATUS_INTERACTED; }
    }
}
