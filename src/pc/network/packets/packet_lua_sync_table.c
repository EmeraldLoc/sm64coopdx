...
void network_send_lua_sync_table(u8 toLocalIndex, u64 seq, u16 modRemoteIndex, u16 lntKeyCount,
                                 struct LSTNetworkType *lntKeys, struct LSTNetworkType *lntValue) {
    if (gLuaState == NULL) { return; }
    if (lntKeyCount >= MAX_UNWOUND_LNT) {
        LOG_ERROR("Tried to send too many lnt keys");
        return;
    }
...
void network_receive_lua_sync_table(struct Packet *p) {
    ...
    if (lntKeyCount >= MAX_UNWOUND_LNT) {
        LOG_ERROR("Tried to receive too many lnt keys");
        return;
    }
