    ...
    packet_ordered_begin();
    {
        ...
        for (struct SyncObject *so = sync_object_get_first(); so != NULL; so = sync_object_get_next()) {
            if (so == NULL || so->o == NULL
                || so->o->behavior != smlua_override_behavior(bhvRespawner)) {
                continue;
            }
...
void network_receive_area(struct Packet *p) {
    ...
    if (levelControlTimerVisible) { level_control_timer(TIMER_CONTROL_SHOW); }
    if (levelControlTimerRunning) { level_control_timer(TIMER_CONTROL_START); }
    ...
    for (u32 i = 0; i < sRemoveSyncIdsIndex; i++) {
        ...
        if (so->o != NULL) { so->o->activeFlags = ACTIVE_FLAG_DEACTIVATED; }
    ...
    for (s32 i = 0; i < respawnerCount; i++) {
        ...
        if (so == NULL) {
            ...
            LOG_DEBUG("rx area debug: Sync Object DEBUG:\n\n \
                       ...
                       RESPAWN BEHAVIOR: %u\n \
                       SYNC ID: %u\n\n",
                      posX, posY, posZ, behParams, respawnerModelToRespawn, respawnerMinSpawnDist,
                      behaviorToRespawn, syncId);
            continue;
        }
        ...
        if (syncId < SYNC_ID_BLOCK_SIZE) {
            struct Object *respawner = spawn_object_abs_with_rot(
                gMarioStates[0].marioObj, 0, MODEL_NONE, bhvRespawner, posX, posY, posZ, 0, 0, 0);
            if (respawner != NULL) {
