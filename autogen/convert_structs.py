...
out_filename_docs_rtd = 'docs/lua/references/structs_reference/'
...
def doc_struct_field(struct, field):
    ...
    if ftype == cobject_function_identifier:
        ...
        if [cli_opts['rtd']]:
            return '| %s | [`%s`](%s) |\n' % (fid, field['function'], flink.replace('structs.md#', '') + '.md'), True
        else:
    ...
    if flink:
        if [cli_opts['rtd']]:
            return '| %s | [%s](%s) | %s |\n'  % (fid, ftype, flink.replace('structs.md#', '') + '.md', restrictions), False
        else:
...
def doc_structs_rtd(structs):
    structs.extend(parse_structs(lua_manually_defined_structs, False))
    structs = sorted(structs, key=lambda d: d["identifier"])
    ...
    for struct in structs:
        ...
        doc = doc_struct(struct)

        filename = struct["identifier"] + ".md"

        with open(
            get_path(out_filename_docs_rtd + filename),
            "w",
            encoding="utf-8",
            newline="\n",
        ) as out:
