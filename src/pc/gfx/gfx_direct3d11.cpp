...
static void gfx_d3d11_init(void) {
    ...
    vertex_buffer_desc.ByteWidth = 256 * ((16 + (CC_MAX_INPUTS * 4) + (2 * 2)) * 3) * sizeof(float); // Same as buf_vbo size in gfx_pc
...
static struct ShaderProgram *gfx_d3d11_create_and_load_new_shader(struct ColorCombiner* cc) {
    ...
    ied[ied_index++] = { "TEXCOORDTWO", 0, DXGI_FORMAT_R32G32_FLOAT, 0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 };
