...
void sync_objects_update(void) {
    ...
    while (entry) {
        ...
        if (entry->forgetTimer == FORGET_TIMEOUT) {
            ...
            if (currentSo == entry->so) { hmap_del(sSoMap, entry->so->id); }
        }
        ...
        if (entry->forgetTimer-- <= 0) {
            ...
            // get_behavior_name_from_id(get_id_from_behavior(entry->so->behavior)));
            free(entry->so);
            free(entry);
...
void sync_object_forget(u32 syncId) {
    ...
    if (so != NULL && so->o != NULL && so->o->oSyncID < SYNC_ID_BLOCK_SIZE) {
        ...
        if (so == so2) { so->lastReliablePacket.error = true; }
    ...
    // get_behavior_name_from_id(get_id_from_behavior(so->behavior)));
}
...
struct SyncObject *sync_object_init(struct Object *o, float maxSyncDistance) {
    ...
    if (o->coopFlags & COOP_OBJ_FLAG_NON_SYNC) { return NULL; }

    bool hadSyncId = (o->oSyncID != 0);
    // generate new sync ID
    if (!sync_object_set_id(o)) {
        LOG_ERROR("failed to set sync id for object w/behavior %d (init_object)",
    ...
    if (o->coopFlags & COOP_OBJ_FLAG_INITIALIZED) { return so; }
...
bool sync_object_should_own(u32 syncId) {
    ...
    for (s32 i = 0; i < MAX_PLAYERS; i++) {
        ...
        if (player_distance(&gMarioStates[0], so->o) > player_distance(&gMarioStates[i], so->o)) {
            return false;
        }
    }
...
bool sync_object_set_id(struct Object *o) {
    ...
    if (syncId == 0) {
        if (!gNetworkAreaLoaded) {
            ...
            if (o->parentObj != NULL && o->parentObj->oSyncID > 0
                && (o->parentObj->oSyncID % 10) == 0) {
                for (s32 i = 0; i < 10; i++) {
    ...
    if (syncId == 0 || !ctx) {
        o->oSyncID = 0;
        LOG_ERROR("failed to set sync id for object w/behavior %d (set_sync_id) %u",
                  get_id_from_behavior(o->behavior), gNetworkAreaLoaded);
        return !ctx;
    }
    ...
    if (!so) {
        LOG_ERROR("failed to set sync id (o) for object w/behavior %d (set_sync_id) %u, ctx %u",
                  get_id_from_behavior(o->behavior), gNetworkAreaLoaded, ctx);
        return false;
    }
    ...
    if (gNetworkAreaLoaded) {
        LOG_INFO("set sync id for object w/behavior %d, ctx %u", get_id_from_behavior(o->behavior),
                 ctx);
    }
