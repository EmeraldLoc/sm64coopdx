...
static void player_changed_level(struct NetworkPlayer *np, s16 courseNum, s16 actNum, s16 levelNum,
                                 s16 areaIndex) {
    ...
    struct NetworkPlayer *npLevelAreaMatch =
        get_network_player_from_area(courseNum, actNum, levelNum, areaIndex);
...
void network_send_change_level(void) {
    ...
    if (gCurrCourseNum == 0
        && (gCurrLevelNum == LEVEL_CASTLE || gCurrLevelNum == LEVEL_CASTLE_GROUNDS
    ...
    if (gNetworkType == NT_SERVER) {
        player_changed_level(gNetworkPlayerLocal, gCurrCourseNum, gCurrActStarNum, gCurrLevelNum,
                             gCurrAreaIndex);
        return;
    }
    ...
    np->currAreaSyncValid = false;
    np->currLevelSyncValid = false;
    network_player_update_course_level(np, gCurrCourseNum, gCurrActStarNum, gCurrLevelNum,
                                       gCurrAreaIndex);
