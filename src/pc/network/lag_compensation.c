...
void lag_compensation_store(void) {
    ...
    if (sLocalStateHistoryIndex + 1 >= MAX_LOCAL_STATE_HISTORY) { sLocalStateHistoryReady = true; }
...
struct MarioState *lag_compensation_get_local_state(struct NetworkPlayer *otherNp) {
    ...
    if (pingToTicks > (MAX_LOCAL_STATE_HISTORY - 1)) { pingToTicks = (MAX_LOCAL_STATE_HISTORY - 1); }
