...
static bool allowable_behavior_change(struct SyncObject *so, BehaviorScript *behavior) {
    ...
    bool oBehaviorPenguin = (o->behavior == smlua_override_behavior(bhvPenguinBaby)
                             || o->behavior == smlua_override_behavior(bhvSmallPenguin));
    bool inBehaviorPenguin = (behavior == smlua_override_behavior(bhvPenguinBaby)
                              || behavior == smlua_override_behavior(bhvSmallPenguin));

    // bhvCoinFormationSpawn can be set to bhvYellowCoin
    bool oBehaviorCoin = (o->behavior == smlua_override_behavior(bhvCoinFormationSpawn)
                          || o->behavior == smlua_override_behavior(bhvYellowCoin));
    bool inBehaviorCoin = (behavior == smlua_override_behavior(bhvCoinFormationSpawn)
                           || behavior == smlua_override_behavior(bhvYellowCoin));
...
static struct SyncObject *packet_read_object_header(struct Packet *p, u8 *fromLocalIndex) {
    ...
    *fromLocalIndex = (np != NULL) ? np->localIndex : p->localIndex;
    ...
    if (so->rxEventId[*fromLocalIndex] > eventId
        && (u16)abs(eventId - so->rxEventId[*fromLocalIndex]) < USHRT_MAX / 2) {
    ...
    if (o->behavior != behavior && o->behavior != lBehavior
        && !allowable_behavior_change(so, behavior)) {
        enum BehaviorId objBehaviorId = get_id_from_behavior(o->behavior);
        enum BehaviorId inBehaviorId = get_id_from_behavior(behavior);
        LOG_ERROR("during read behavior mismatch for %d: %04X (%s) vs %04X (%s)", syncId, objBehaviorId,
                  get_behavior_name_from_id(objBehaviorId), inBehaviorId,
                  get_behavior_name_from_id(inBehaviorId));
        return NULL;
    }
...
void network_send_object(struct Object *o) {
    ...
    if (o->behavior != so->behavior && !allowable_behavior_change(so, so->behavior)) {
        ...
        LOG_ERROR("during send behavior mismatch for %d: %04X (%s) vs %04X (%s)", o->oSyncID,
                  objBehaviorId, get_behavior_name_from_id(objBehaviorId), soBehaviorId,
                  get_behavior_name_from_id(soBehaviorId));
        sync_object_forget(so->id);
        return;
    }

    bool reliable =
        (o->activeFlags == ACTIVE_FLAG_DEACTIVATED || so->maxSyncDistance == SYNC_DISTANCE_ONLY_EVENTS);
...
void network_send_object_reliability(struct Object *o, bool reliable) {
    ...
    if (o->behavior != so->behavior && !allowable_behavior_change(so, so->behavior)) {
        enum BehaviorId objBehaviorId = get_id_from_behavior(o->behavior);
        enum BehaviorId soBehaviorId = get_id_from_behavior(so->behavior);
        LOG_ERROR("during send reliability behavior mismatch for %d: %04X (%s) vs %04X (%s)", syncId,
                  objBehaviorId, get_behavior_name_from_id(objBehaviorId), soBehaviorId,
                  get_behavior_name_from_id(soBehaviorId));
        sync_object_forget(so->id);
        return;
...
void network_receive_object(struct Packet *p) {
    ...
    if (o != NULL && o->collisionData) {
        ...
        for (s32 i = 0; i < MAX_PLAYERS; i++) {
            ...
            for (s32 j = 0; j < 3; j++) {
                gMarioStates[i].pos[j] += deltaPos[j];
            }
        }
    }
}
    ...
    for (struct SyncObject *so = sync_object_get_first(); so != NULL; so = sync_object_get_next()) {
        ...
        if (so->o->oSyncID != so->id) {
            ...
            if (so->o->activeFlags != ACTIVE_FLAG_DEACTIVATED
                ...
                LOG_ERROR("sync id mismatch: %d vs %d (behavior %s, %d)", so->o->oSyncID, so->id,
                          bhvName != NULL ? bhvName : "NULL", bhvId);
            }
            sync_object_forget(so->id);
            continue;
        }
        ...
        if (network_player_any_connected() && !inCredits) { network_send_object(so->o); }
    }
}
