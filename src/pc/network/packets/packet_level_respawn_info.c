...
// TODO: These respawn info changes REALLY need to be batched and sent on a timer instead of
// immediately.
//       currently when collecting coins a flood of packets gets sent out when there is no immediate
//       need to know when a coin is collected in a different area. Ideally this logic would be combined
//       into packet_level_macro and packet_level_spawn_info without being bolted on top like this.
...
static s16 *get_respawn_info_from_macro_offset(u16 areaIndex, u16 macroOffset) {
    ...
    while (macroObjList != NULL && *macroObjList != -1) {
        ...
        if (macroOffset == index) { return respawnInfo; }
        index++;
    }
...
static u32 *get_respawn_info_from_spawn_info_index(u16 areaIndex, u16 fromSpawnInfoIndex) {
    ...
    while (spawnInfo != NULL) {
        if (spawnInfoIndex == fromSpawnInfoIndex) { return &spawnInfo->behaviorArg; }
...
static u16 get_macro_offset_of_object(struct Object *o) {
    ...
    while (macroObjList != NULL && *macroObjList != -1) {
        ...
        if (o->respawnInfo == respawnInfo) { return index; }
        index++;
    }
...
static u16 get_spawn_info_index_of_object(struct Object *o) {
    ...
    while (spawnInfo != NULL) {
        ...
        if (&spawnInfo->behaviorArg == o->respawnInfo) { return spawnInfoIndex; }
        spawnInfo = spawnInfo->next;
        spawnInfoIndex++;
    }
...
void network_send_level_respawn_info(struct Object *o, u8 respawnInfoBits) {
    ...
    packet_write(&p, isMacroObject ? &macroOffset : &spawnInfoIndex, sizeof(u16));
...
void network_receive_level_respawn_info(struct Packet *p) {
    ...
    bool levelMismatch =
        (courseNum != gCurrCourseNum || actNum != gCurrActStarNum || levelNum != gCurrLevelNum);
    if (gNetworkType == NT_SERVER) {
